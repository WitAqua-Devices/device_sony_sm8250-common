...
void vendor_load_properties() {
    ...
    // Setting ro.product.model here stops property_initialize_ro_product_props()
    // from deriving it, so override the per-partition ones for consistency too.
    InitPropertySet("ro.product.model", model);
    for (const char* partition : {"odm", "product", "system", "system_ext", "vendor"}) {
        InitPropertySet(std::string("ro.product.") + partition + ".model", model);
    }
}
